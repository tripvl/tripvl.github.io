  ...
  return {
    ...
    render(info) {
      ...
      el.values.textContent = [
        ...
        `yawOffset      ${f(info.yawOffset)}° (${info.yawOffsetInitialized ? 'готов' : 'не нужен'})`,
        `склонение      ${f(info.declination)}° (${info.magneticModel})`,
        `режим          ${info.displayMode || '—'}`,
        `курс экрана    ${f(info.screenHeading)}°`,
        `нижний компас  ${info.downCompassActive ? 'да' : 'нет'}`,
        `азимут пойман  ${info.compassAligned ? 'да' : 'нет'}`,
        ``,
        ...
        `бёрст Персеид  ${info.meteorBurstActive ? 'идёт' : 'нет'}`,
        `прогресс       ${f((info.meteorBurstProgress ?? 0) * 100, 0)}%`,
        `повтор готов   ${info.meteorBurstReady ? 'да' : 'нет'}`,
        ``,
